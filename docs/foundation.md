...
*   **核心职责**：
    ...
    *   **图片识别服务**：接收图片 Asset 进行云端 OCR / VLM 识别；MVP 主实现使用 DashScope 多模态模型，模型名通过 `DASHSCOPE_IMAGE_MODEL` 配置，识别结果写回 `image.props.caption`。
...
| 当前状态 | 触发 | 下一状态 | 说明 |
|---|---|---|---|
| `pending_upload` | `POST /assets/upload` 创建 | `pending_upload` | 服务端生成 API 上传代理地址，文件尚未确认上传 |
| `pending_upload` | 客户端通过 API 上传代理完成所有 part | `uploaded` | 服务端保存 part bytes 和 part metadata |
| `uploaded` | 服务端完成元数据解析或安全检查 | `ready` | Block 只能引用 `ready` 状态的 Asset |
| `pending_upload` / `uploaded` | 校验失败、超时或内容缺失 | `failed` | 客户端可重新创建 Asset 上传 |

`pending_upload` 超过 24 小时未完成时，服务端可以自动标记为 `failed`。`ready` Asset 被 Block 引用后不能直接删除，必须先删除或改写引用它的 Block。

上传协议：MVP 统一使用后端 API multipart 上传代理，不再使用对象存储直传。图片和小文件也可以只有 1 个 part；音频默认分片大小为 5MB。客户端本地必须保存 `asset_id`、`upload_id` 和已完成的 `part_number`，用于弱网断点续传。`ASSET_UPLOAD_URL_MODE="api"` 是当前唯一支持模式。
...
```json
{
  ...
  "image": { "asset_id": "asset_photo1", "caption": "白板上的架构图", "width": 1280, "height": 720, "recognition_task_id": "task_456", "recognition_generated_at": "2026-07-14T10:02:00Z" },
...
```json
{
  ...
  "name": "产品组",
  "invite_code": "123456",
  "invite_code_expires_at": "2026-07-15T10:00:00Z",
  "member_count": 2,
  "role": "owner",
  "created_at": "2026-07-14T10:00:00Z",
...
`role` 为当前用户在该组内的角色，MVP 取值为 `owner`、`member`。`invite_code` 固定为 6 位数字，创建组时生成，有效期 24 小时。口令过期只影响新成员加入，不影响已经加入的成员访问组内消息。

Group Document Message 结构：
...
```json
{
  ...
  "document_title": "会议纪要",
  "document_revision": 3,
  "sent_at": "2026-07-14T10:06:00Z"
}
```

发送文档到组时，服务端必须保存发送时的 Document 快照。后续原 Document 被编辑或删除，不影响已发送消息的下载结果。组成员下载的是消息快照，不获得原始 `documents` 表访问权。
...
`task.type` 取值：`convert_manuscript`、`asr_audio`、`recognize_image`、`export_document`、`ai_rewrite`。`task.status` 取值：`queued`、`processing`、`succeeded`、`failed`、`cancelled`。
...
```json
{
  ...
  "parts": [
    {
      "part_number": 1,
      "upload_url": "https://api.example/api/v1/assets/asset_rec1/upload-parts/1?upload_id=upload_123&expires_at=1784024100&signature=opaque_signature",
      "headers": { "Content-Type": "audio/mp4" },
      "expires_at": "2026-07-14T10:15:00Z"
    },
    {
      "part_number": 2,
      "upload_url": "https://api.example/api/v1/assets/asset_rec1/upload-parts/2?upload_id=upload_123&expires_at=1784024100&signature=opaque_signature",
      "headers": { "Content-Type": "audio/mp4" },
      "expires_at": "2026-07-14T10:15:00Z"
    }
  ]
...
客户端断点续传时先调用该接口，跳过 `uploaded_parts`，只上传 `missing_parts`。API 上传代理 URL 过期时也通过该接口重新获取。

`PUT /assets/{asset_id}/upload-parts/{part_number}`

Query：`upload_id`、`expires_at`、`signature` 来自 `upload_url`。

Request Body：原始二进制 part bytes，`Content-Type` 使用 `headers` 中返回的值。
...
Response `200`：返回完整 Asset 结构。服务端必须校验 `parts` 连续完整、总大小等于 `size_bytes`、API 上传代理保存的 part ETag 匹配、最终文件 `checksum_sha256` 匹配。校验失败返回 `422 validation_error`，Asset 状态变为 `failed`。
...
Response `200`：通过后端 API 直接返回文件 bytes，响应头包含 `Content-Type`、`Content-Length`、`Content-Disposition` 和 `Accept-Ranges`。权限不足返回 `403`。
...
Group API 用于 PC 端首页“组”功能。MVP 不是多人实时协作，只提供基于 6 位口令的组加入、组内文档消息和文档快照下载。
...
Response `201`：返回 Group 结构。服务端生成 `group_` ID、6 位数字 `invite_code` 和 24 小时后的 `invite_code_expires_at`，并把当前用户加入为 `owner`。
...
Response `200`：返回加入后的 Group 结构。用户已在组内时，重复提交同一口令返回当前组结构，不报错。口令不存在返回 `404 not_found`；口令过期返回 `403 forbidden`。

`GET /groups?limit=20&cursor=...`

Response `200`：返回当前用户已经加入的组列表。
...
`GET /groups/{group_id}/messages?limit=20&cursor=...`

Response `200`：返回组内文档消息列表。只有组成员可访问，非成员返回 `403 forbidden`。
...
Response `201`：返回 Group Document Message 结构。服务端必须校验当前用户是组成员，并且当前用户拥有该 Document。发送成功时保存当前 Document 快照，组内消息引用该不可变快照。

`GET /groups/{group_id}/documents/{message_id}/download?format=docx`

Response `200`：返回文件 bytes。`format` 取值为 `pdf`、`docx`，默认 `docx`。响应头必须包含 `Content-Type`、`Content-Length`、`Content-Disposition: attachment`。服务端必须校验当前用户是组成员；下载内容来自消息保存的 Document 快照，不读取发送者当前 Document。

Group 权限规则：

*   创建组者为 `owner`，通过口令加入者为 `member`。
*   MVP 不提供踢人、退组、转让 owner、刷新口令和实时协作接口。
*   只有组成员能看消息、发送文档到组和下载组内文档消息。
*   发送文档要求发送者拥有该 Document；下载组消息不要求下载者拥有原始 Document。

#### 8. 删除与级联 API

`DELETE /documents/{document_id}`

Response `204`：硬删除 Document。删除 Document 不会删除关联 Manuscript 和 Asset。已经生成的 export 文件作为 `export` Asset 保留，后续由服务端生命周期任务清理。
...
| `audio.props.transcript` | 一个或多个 `paragraph`，段首固定 `发言：` | 按 `speaker_segments.start_ms` 升序；无 speaker_segments 时按 transcript 原文顺序；`optimize_audio=true` 时先清理口水词和无意义重复 | 每个 paragraph 指向对应 audio Block；有时间段时写入 `range` |
| `handwriting` | `heading` / `paragraph`，必要时额外生成 `image` | 保持原 handwriting Block 在时间线中的位置；先插入可保留手绘图片，再插入识别文本 | 指向 handwriting Block；有区域识别时写入 `region`，否则 `region: null` |
| `image` | `image` | 保持原 image Block 在时间线中的位置；caption 为空时转换任务内可请求图片描述，但不回写 Manuscript | 指向 image Block；caption 从 `image.props.caption` 或转换任务内 AI 识别结果生成 |
...
*   转换任务基于提交时的 Manuscript 快照生成新 Document。除创建 Task 和 Document 外，转换任务不修改源 Manuscript。
*   如果原 Document 已被用户编辑，重新转换不会合并到该 Document；“覆盖原 Document”属于 P1+ 能力。
*   LLM / VLM 只能输出符合 `DocumentBlock.type` 和 `props` 枚举的结构；无法可靠识别的内容降级为 `paragraph` 纯文本、保留原图或跳过，并在 `task.result.warnings` 中记录。
...
`optimize_audio` 是必填 boolean。未开启录音优化时客户端必须传 `false`。

Response `202`：立即返回 `queued` 任务结构。客户端通过 `GET /tasks/{task_id}` 轮询进度；任务成功后的 `result`：
...
`warnings` 可省略。`convert-manuscript` 使用后台任务执行，进度阶段使用 `queued`、`llm_parse`、`document_build`、`completed`。成功但部分 Block 降级时任务仍为 `succeeded`，降级详情写入 `result.warnings`。

固定 warning code：`audio_transcript_missing`、`audio_optimization_failed`、`image_caption_failed`、`handwriting_empty`、`handwriting_render_failed`、`handwriting_recognition_failed`。
...
Response `202`：返回任务结构。任务成功后 `result` 使用 `recognize_image` 结果结构。

该接口是非流式兼容入口。服务端创建 `recognize_image` task 后异步执行图片 OCR / VLM 识别，客户端通过 `GET /tasks/{task_id}` 轮询任务状态。识别成功后，服务端必须覆盖所有引用该 `asset_id` 的 Manuscript `image` Block 中的 `props.caption`，并写入 `props.recognition_task_id`、`props.recognition_generated_at`。MVP 不保存多版本图片识别结果。

`POST /tasks/recognize-image/stream`

Request：与 `POST /tasks/recognize-image` 完全一致。

Response `200`，`Content-Type: text/event-stream`。该接口是图片识别的流式主入口。服务端创建 `recognize_image` task 后立即开始识别，并通过 SSE 返回任务状态、增量文本和最终任务结果。客户端收到 `done` 后应以 `task.result` 为最终结果；如果需要和服务端 Manuscript revision 对齐，应重新拉取 Manuscript 或对应 blocks 分页接口。

SSE 事件格式：

```text
event: task
data: {"task":{"id":"task_456","type":"recognize_image","status":"queued","progress":{"stage":"queued","current":0,"total":1,"message":"Image recognition task queued."},"result":null,"error":null,"retry_count":0,"billing":null,"created_at":"2026-07-14T10:00:00Z","updated_at":"2026-07-14T10:00:00Z"}}

event: task
data: {"task":{"id":"task_456","type":"recognize_image","status":"processing","progress":{"stage":"llm_parse","current":0,"total":1,"message":"Image recognition task processing."},"result":null,"error":null,"retry_count":0,"billing":null,"created_at":"2026-07-14T10:00:00Z","updated_at":"2026-07-14T10:00:01Z"}}

event: delta
data: {"task_id":"task_456","text":"白板上的三端架构草图","caption":"白板上的三端架构草图","recognized_text":"白板上的三端架构草图"}

event: done
data: {"task":{"id":"task_456","type":"recognize_image","status":"succeeded","progress":{"stage":"completed","current":1,"total":1,"message":"Image recognition task completed."},"result":{"asset_id":"asset_photo1","caption":"白板上的三端架构草图","text":"白板上的三端架构草图\n移动端、PC 端和后端 API 协作。"},"error":null,"retry_count":0,"billing":null,"created_at":"2026-07-14T10:00:00Z","updated_at":"2026-07-14T10:00:03Z"}}
```
...
*   MVP 默认 `VISION_PROVIDER=dashscope`，使用阿里云百炼 DashScope 多模态模型。
*   图片模型名通过 `DASHSCOPE_IMAGE_MODEL` 配置，默认 `qwen-vl-plus`。
*   DashScope 输入图片使用 Base64 data URI：`data:<content_type>;base64,<payload>`。服务端必须在调用前校验 Base64 后 payload 不超过 10MB，超过返回失败任务，`error.code` 使用 `validation_error`，`retryable=false`。
*   识别结果的第一行作为 `caption`，完整输出作为 `text`。客户端展示摘要时优先使用 `caption`，需要详细 OCR / VLM 内容时使用 `text`。
*   图片识别失败不得删除图片 Asset 或 image Block，客户端应允许用户稍后重试或手工编辑 caption。
...
```json
{
  "download_url": "https://api.example/api/v1/assets/asset_export1/stream",
  "expires_at": "2026-07-14T10:15:00Z"
}
```

服务端必须在生成 `download_url` 前校验当前用户对 `document_id` 的访问权限。MVP 的 `download_url` 指向后端 API stream 地址，不再指向对象存储签名下载地址。
...
| `GET /groups` | `updated_at,id` | `desc` | opaque，服务端内部可编码 `updated_at + id` |
| `GET /groups/{id}/messages` | `sent_at,id` | `desc` | opaque，服务端内部可编码 `sent_at + message_id` |
...
GET /groups/{id}/documents/{message_id}/download
GET /documents/{id}
...
图片识别重跑规则：MVP 不保存多版本图片识别。二次识别成功后覆盖 `image.props.caption`，并写入 `image.props.recognition_task_id`、`image.props.recognition_generated_at`。非流式和流式图片识别入口遵循相同覆盖规则；流式入口只改变客户端获取增量文本的方式，不改变最终持久化语义。图片识别多版本、区域级 OCR 和人工对比属于 P1。
...
```json
{
  ...
  "blocks": [
    ...
    },
    {
      ...
      "platform": "mac",
      "deleted": false,
      "props": { "asset_id": "asset_photo1", "caption": "白板上的架构图", "width": 1280, "height": 720, "recognition_task_id": "task_456", "recognition_generated_at": "2026-07-14T10:02:00Z" }
    },
    {
      "id": "block_text1",
      "type": "text",
...
*   **缓存与队列**：Redis
*   **文件存储**：MVP 使用 PostgreSQL 保存上传 part 和最终 Asset bytes，通过后端 API 上传代理与 stream 下载；不引入对象存储
...
| 文件存储 | PostgreSQL BYTEA / JSONB part contents + 后端 API 上传代理；MVP 不使用对象存储 |
